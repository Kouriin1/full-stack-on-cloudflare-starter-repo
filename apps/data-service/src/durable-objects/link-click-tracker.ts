import { deleteClicksBefore, getRecentClicks } from '@/hono/helpers/durable-queries';
...
export class LinkClickTracker extends DurableObject {
    ...
    constructor(ctx: DurableObjectState, env: Env) {
        ...
        ctx.blockConcurrencyWhile(async () => {
            ...
            this.sql.exec(`
                CREATE TABLE IF NOT EXISTS geo_clicks (
    ...
    async addClick(latitude: number, longitude: number, country: string, time: number) {
		this.sql.exec(
			`
			INSERT INTO geo_clicks (latitude, longitude, country, time)
			VALUES (?, ?, ?, ?)
			`,
			latitude,
			longitude,
